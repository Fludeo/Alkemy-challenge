...
const EditRecordForm =({closeForm,handleEditSubmit,UpdateForm, formData ,type}:props) =>{
    ...
    <form className="edit-record-form">
        ...
        <input defaultValue={formData.concept} onChange={(e)=>UpdateForm({...formData, concept: e.currentTarget.value.trim()})} 
        className='edit-record-form__input' type="text"  />

        <label className='edit-record-form__label' >Amount</label>
        <input defaultValue={formData.amount} type='number' step={0.01} min={1} onChange={(e)=>UpdateForm({...formData, amount: Number(e.currentTarget.value)})} 
        className='edit-record-form__input' />
        <label className='edit-record-form__label' >Category</label>
        <select onChange={(e)=>UpdateForm({...formData,category:e.currentTarget.value})} defaultValue={formData.category} className='edit-record-form__input'>
        ...
        <input 
        defaultValue={FormatDate(formData.date)} 
        onChange={(e)=>UpdateForm({...formData, date: new Date(e.currentTarget.value)})} 
        className='edit-record-form__input' type="date"  />
        <p className='edit-record-form__error'>{formData.errorMessage}</p>
         <div className='edit-record-form__button-container'>
         <button onClick={handleEditSubmit} className='edit-record-form__accept-button edit-record-form__accept-button--hover'>Accept</button>
